...
class Enter_car(object):
    ...
    def WarningWindow_duplicate(self):
        ...
        label2 = tkinter.Label(self.window, text='No Slots are available!!', bg='#FA8072')
        label2.pack(side='top')
    ...
    def CheckInfo(self):
        if self.carColorEntry.get() and self.carTypeEntry.get() and self.carNumberEntry.get() and self.cardTypeEntry.get() and self.cardNumberEntry.get():
            self.info()
            returned = self.storeInfoInDB()
            if returned == False:
                return self.WarningWindow_duplicate()
...
# # Added this: ----------------------------
# myc.execute("SELECT * FROM parkingInfo")
# rows = myc.fetchall()
# for row in rows:
#   print(row)
# # Remove this: ----------------------------